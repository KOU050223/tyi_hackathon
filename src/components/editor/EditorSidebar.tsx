      ...
      <div className="mt-2">
        {user ? (
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full px-4 py-3 border border-[#00FF00] text-[#00FF00] font-mono text-sm hover:bg-[#00FF00]/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {saving ? 'Saving...' : id ? 'Update Pattern' : 'Save Pattern'}
          </button>
        ) : (
          <div className="text-center p-3 border border-[#FFFF00]/30 text-[#FFFF00] text-sm">
            保存するにはログインしてください
          </div>
        )}
        {error && (
